    ...
    const loadData = async () => {
      try {
        ...
        if (cachedData) {
          ...
          // Trouver la catégorie Energie et récupérer les vidéos correspondantes
          const energieCategory = parsedData.find(category => category.name === 'Energie');
          if (energieCategory) {
            setFilteredVideos(energieCategory.videos);
          }
        } else {
        ...
        const vipStatus = await AsyncStorage.getItem('isVIPEnergie');
  ...
  const handleSearch = (text) => {
    ...
    if (text) {
      const filtered = categories
        .find(category => category.name === 'Energie')?.videos
        .filter(video =>
          video.title.toLowerCase().includes(text.toLowerCase())
        );
      setFilteredVideos(filtered);
    } else {
      const energieCategory = categories.find(category => category.name === 'Energie');
      setFilteredVideos(energieCategory?.videos || []);
    }
  };
